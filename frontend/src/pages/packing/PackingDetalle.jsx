...
const PackingDetalle = ({ packing, onClose, onEdit }) => {
  ...
  const handleCrearSubasta = async () => {
    if (
      !subastaForm.fecha_hora_inicio ||
      !subastaForm.duracion_horas ||
      !subastaForm.precio_base
    ) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <Badge
              variant={
                ESTADO_COLORS[packing.estado_actual || packing.estado] ||
                "default"
              }
            >
              {packing.estado_display ||
                packing.estado_actual ||
                packing.estado}
            </Badge>
          </div>
          <div>
            ...
            <p className="text-sm font-medium text-gray-900">
              {formatDateShort(packing.fecha_inicio_semana)} -{" "}
              {formatDateShort(packing.fecha_fin_semana)}
            </p>
          </div>
          <div>
              ...
              >
                ...
                <div className="bg-blue-50 px-4 py-3 flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-blue-700">
                    {formatKg(
                      tipo.kg_total || calcularTotalTipo(tipo.detalles),
                    )}{" "}
                    kg
                  </span>
                </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 table-fixed">
                    ...
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="w-[25%] px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">
                          PY
                        </th>
                        <th className="w-[20%] px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">
                          Estado Subasta
                        </th>
                        {/* Usamos un ancho fijo en Acción para que los botones quepan siempre igual */}
                        <th className="w-[20%] min-w-[120px] px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">
                          Acción
                        </th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {tipo.detalles.map((detalle, index) => {
                        ...
                        return (
                          ...
                          >
                            {/* 3. Asegúrate de que los td mantengan truncate si el texto es muy largo */}
                            ...
                            <td className="px-4 py-2 text-center">
                              {subasta ? (
                                <Badge
                                  variant={
                                    SUBASTA_COLORS[subasta.estado_actual]
                                  }
                                >
                                  {SUBASTA_LABELS[subasta.estado_actual] ||
                            ...
                            <td className="px-4 py-2 text-center">
                              {/* Usamos un flex contenedor con ancho fijo para que la columna no se mueva */}
      ...
      <div className="text-xs text-gray-400 border-t pt-4">
        <p>
          Registrado:{" "}
          {packing.fecha_registro
            ? new Date(packing.fecha_registro).toLocaleString("es-PE")
            : "-"}
        </p>
        {packing.fecha_actualizacion && (
          <p>
            Última actualización:{" "}
            {new Date(packing.fecha_actualizacion).toLocaleString("es-PE")}
          </p>
        )}
      </div>
        ...
        {detalleParaSubasta && (
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-semibold">
                    {detalleParaSubasta.tipo_fruta_nombre}
                  </div>
                </div>
                <div>
                  <span className="text-blue-600">Día:</span>
                  <div className="font-semibold">
                    {getDiaLabel(detalleParaSubasta.dia)}
                  </div>
                </div>
                <div>
                  <span className="text-blue-600">Fecha:</span>
                  <div className="font-semibold">
                    {detalleParaSubasta.fecha}
                  </div>
                </div>
                <div>
                  <span className="text-blue-600">Kilos:</span>
                  <div className="font-semibold text-green-600">
                    {formatKg(detalleParaSubasta.py)} kg
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              <Button
                onClick={
                  modoEdicion ? handleGuardarSubasta : handleCrearSubasta
                }
        ...
        {subastaSeleccionada && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <Badge
                variant={SUBASTA_COLORS[subastaSeleccionada.estado_actual]}
              >
                {SUBASTA_LABELS[subastaSeleccionada.estado_actual] ||
                  subastaSeleccionada.estado_actual}
              </Badge>
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="font-semibold">
                    {getDiaLabel(subastaSeleccionada.detalle?.dia)}
                  </div>
                </div>
                <div>
                  <span className="text-blue-600 block">Fecha Producción</span>
                  <div className="font-semibold">
                    {subastaSeleccionada.detalle?.fecha}
                  </div>
                </div>
                <div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {new Date(
                        subastaSeleccionada.fecha_hora_inicio,
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {new Date(
                        subastaSeleccionada.fecha_hora_fin,
                      ).toLocaleString("es-PE", {
                        weekday: "short",
                        day: "2-digit",
                        month: "short",
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      S/{" "}
                      {parseFloat(subastaSeleccionada.precio_base).toFixed(2)}
                    </span>
                  </div>
                  {subastaSeleccionada.precio_actual && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-green-600 text-lg">
                        S/{" "}
                        {parseFloat(subastaSeleccionada.precio_actual).toFixed(
                          2,
                        )}
                      </span>
                    </div>
                  ...
                  <div className="flex justify-between border-t pt-2">
                    ...
                    <span className="font-medium">
                      {subastaSeleccionada.total_ofertas || 0}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-yellow-700">
                    {subastaSeleccionada.cliente_ganando.nombre}
                  </span>
                  <span className="font-bold text-yellow-800 text-lg">
                    S/{" "}
                    {parseFloat(
                      subastaSeleccionada.cliente_ganando.monto,
                    ).toFixed(2)}
                  </span>
                </div>
              </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              <Button
                variant="secondary"
                onClick={() => setShowDetalleSubastaModal(false)}
              >
                Cerrar
              </Button>
              ...
              <Button
                onClick={() =>
                  window.open(
                    `/dashboard/subastas?id=${subastaSeleccionada.id}`,
                    "_self",
                  )
                }
              >
